export class TextHighlighter {
  constructor(hoverableDivId, outputId, outputHoverId) {
    ...
    this.text = this.hoverableDiv.textContent.trim();

    this.context.font = `${this.fontSize} ${this.fontFamily}`;
    this.mouseCol = 0;
    this.mouseColSafe = 0;
    this.relativeY = 0;
    this.contentTextCleaned = this.text.replace(/\t/g, "").replace(/\n/g, " ");
  ...
  positionFloatingComment(element) {
    ...
    let top = this.findYValueFromIndex(this.wordStats, startId);
    // let yColStart = findYValueFromIndex(wordStats, startId)
    // let yColEnd = findYValueFromIndex(wordStats, endId)
    let yCol1 = this.findColFromIndex(this.wordStats, startId)
    let yCol2 = this.findColFromIndex(this.wordStats, endId)
      ...
      if (yCol1 != yCol2) {
        ...
        if (!this.floatingDivsSplit.has(splitId)) {
          ...
          console.log(`yColIndex ${this.findStartIndexFromIndex(this.wordStats, testStartIndex)} xcol ${this.findXValueFromIndex(
            ...
            this.contentTextCleaned,
            testStartIndex
          )} top${this.findYValueFromIndex(this.wordStats, testStartIndex)} startIndex${testStartIndex} `)
          const floatingDiv = document.createElement("div");
  ...
  findColFromIndex(updatedWordStats, startLetterIndex) {
    let previousValue = null;
    let lastSize = updatedWordStats[updatedWordStats.length - 1][1]
    if (lastSize < startLetterIndex) {
      return lastSize
  ...
  findYValueFromIndex(wordStats, startLetterIndex) {
    let previousValue = null;
    let lastSize = wordStats[wordStats.length - 1][1]
    if (lastSize < startLetterIndex) {
      return ((wordStats.length - 1) * this.textAreaYSections) + this.divStartY;
    }
