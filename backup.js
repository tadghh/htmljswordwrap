...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function findXValueFromIndex(yColIndex, mainText, startIndex) {
    let cumulativeWidth = 0;
    ...
    if (element.id.includes("floating-highlighted")) {
      if (yCol1 != yCol2) {
        ...
        if (!floatingDivsSplit.has(uniqueId)) {
          ...
          floatingDiv.setAttribute("end", wordStats[yCol1 + 1][1] - 1)
        ...
        if (!floatingDivsSplit.has(splitId)) {
          ...
          let testStartIndex = wordStats[yCol1 + 1][1]
          let gaycat = findStartIndexFromIndex(wordStats, testStartIndex + 1);

          console.log(selectedText)
          console.log(wordStats)
          console.log(`yColIndex ${findStartIndexFromIndex(wordStats, testStartIndex)} xcol ${findXValueFromIndex(
            gaycat,
            contentTextCleaned,
            testStartIndex
          )} top${findYValueFromIndex(wordStats, testStartIndex)} startIndex${testStartIndex} `)
          const floatingDiv = document.createElement("div");
          floatingDiv.id = splitId;
          floatingDiv.className = "floatingControls";
          floatingDiv.style.width = `${getWordWidth(selectedText)}px`;
          floatingDiv.setAttribute("rawId", rowId)
          floatingDiv.setAttribute("start", wordStats[yCol1 + 1][1] + 1)
          floatingDiv.setAttribute("start2", wordStats[yCol1 + 1][1])
